...
# print to log file
print "PlotData90DaysTemperatureACM0.gp: "\
    .system("date +'%Y-%m-%d %H:%M:%S'")\
    ." : Started ACM0 90 days temperature coefficient plot from "\
...
if (is_missing == 1) {print "PlotData90DaysTemperatureACM0.gp   : ".system("date +'%Y/%M/%d %H:%M:%S'")." : ACM0 3month data file missing, so..."; 
    print "PlotData90DaysTemperatureACM0.gp   : "\
        .system("date +'%Y/%M/%d %H:%M:%S'")\
        ." : **FAILED** to complete ACM0 90 days temperature coefficient plot from "\
        .system("date -d '90 days ago' +'%Y-%m-%d'")\
        ." to "\
        .system("date -d yesterday +'%Y-%m-%d'")
    exit
...
# 90 days temperature coefficient data
pathPlot3 = "/home/pi/UKRAA_PicoMuon/plots/3month/ACM0/".date."_90_days_temperature_coefficient_plot.png"

# Title for graph
GraphTitle3 = "Temperature correction factor for the last 90 days.\n Graph is updated every day at 9.30am \n"
...
set xlabel "Temperature - mean temperature (degC)" 
set xlabel textcolor rgb "black" norotate
set xrange [*:*] noreverse nowriteback

# Y-axis labels and ranges
set ylabel "Change of temperature/mean temperature (%)" 
set ylabel textcolor rgb "dark-violet" rotate
set yrange [*:*] noreverse nowriteback


# least square fit
set print "/home/pi/UKRAA_PicoMuon/data/environment/coefficient/".date."_90_days_temperature_coefficient.txt"            # Define a filename to save the values
c = 0
f(x) = a0*x + c
set fit nolog quiet
fit f(x) FileData using ($5-TEMPERATURE_mean):((($4-MUON_mean)/MUON_mean)*100) via a0
print sprintf("%s,%0.6f,%0.6f,%4.2f", date, a0, a0_err, TEMPERATURE_mean)              # Print the gradient into file
...
set label 2 sprintf("Mean temperature for 90 days: %0.1fdegC", TEMPERATURE_mean)
set label 2 at graph 0.02, 0.90 tc default
set label 3 sprintf("Temperature coefficient: %0.6f+/-%0.5f%", a0, a0_err)
...
plot FileData using ($5-TEMPERATURE_mean):((($4-MUON_mean)/MUON_mean)*100) with points pointsize 3 notitle, \
...
pathPlot = "/home/pi/UKRAA_PicoMuon/temp/ACM0_90_days_temperature_coefficient_plot"
...
print "PlotData90DaysTemperatureACM0.gp: "\
    .system("date +'%Y-%m-%d %H:%M:%S'")\
    ." : Completed ACM0 90 days temperature coefficient plot from "\
    .system("date -d '90 days ago' +'%Y-%m-%d'")\
    ." to "\
    .system("date -d yesterday +'%Y-%m-%d'")
